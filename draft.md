...
#### 4-1 WEB介面
本系統的網頁介面設計基於 HTML、CSS 和 JavaScript 三項核心技術，分別負責系統功能的實現與使用者體驗的提升。

HTML 構建了頁面的結構，設置了主要內容區域和互動按鈕。標籤定義了頁面框架，並為 JavaScript 提供了控制的基礎。使用者介面中的按鈕，負責提交猜測、切換手語錄影、顯示排名等功能，這些設計均旨在提升使用便捷性。

CSS 負責設計的視覺效果和佈局，包括色彩主題、字體樣式及元素的大小和位置。利用 CSS 變數設置色彩主題，Flexbox 和 Grid 技術實現響應式設計，保證不同設備上的顯示效果一致。CSS 媒體查詢則根據螢幕尺寸調整頁面元素的顯示方式，增強了使用者體驗。

JavaScript 則提供動態行為和互動功能，處理使用者操作事件並更新頁面內容。在遊戲模式下，JavaScript 根據用戶的猜測動態更新提示，並計算遊戲得分。介面中的互動按鈕、手語錄影功能、提示顯示和得分統計均由 JavaScript 控制，確保即時反應和良好的互動性。

綜合使用這三項技術，本系統成功實現了一個功能全面且具吸引力的網頁介面，大幅提升了使用者體驗。
