...
#### 4-1 WEB介面
本系統的網頁介面設計使用了三項核心技術：HTML、CSS 和 JavaScript，各自負責不同的功能以實現系統需求並提升使用者體驗。

HTML 用於建立頁面結構，包括設置主要內容區域和互動按鈕。透過標籤構建頁面框架，並創建可互動按鈕，這些按鈕的功能由 JavaScript 控制。使用者介面中，按鈕被用來提交猜測、切換手語錄影以及顯示排名等功能，這些元素的排列和結構設計旨在提高使用便利性。

CSS 負責網頁的視覺效果和佈局，通過設定色彩主題、字體樣式以及元素的大小和位置來優化使用者介面。使用 CSS 變數來設定色彩主題，Flexbox 和 Grid 技術則實現響應式設計，確保在不同設備上顯示效果一致。CSS 媒體查詢用於調整頁面元素在不同螢幕尺寸上的顯示，以提升使用者體驗。例如，在較小螢幕上，元素會自動調整排列方式以適應顯示空間。

JavaScript 提供動態行為和互動功能，負責處理使用者操作事件和更新頁面內容。在遊戲模式下，JavaScript 會根據用戶的猜測動態更新顯示提示，並計算遊戲得分。介面中的互動按鈕、手語錄影功能、提示顯示和得分統計等都是由 JavaScript 控制，以確保即時反應和互動性。

綜合使用這三項技術，本系統成功實現了一個功能全面且具吸引力的網頁介面，提升了使用者體驗，並達成了設計目標。

#### 4-2 遊戲邏輯設計
本系統整合手語識別技術，提供兩種模式：教學模式與 Wordle 模式，以促進學習和娛樂的結合。
